        ...
        if (!this.dbHistory && window.EdgeAPI && EdgeAPI.apiKey) {
            try {
                ...
                if (response.ok) {
                    ...
                    // Filter out steps that have at least one tank defined AND have valid data (non-zero TDS)
                    const filtered = Object.values(grouped).filter(g => {
                        const hasTank = g.ro_reject || g.rainwater || g.blended;
                        if (!hasTank) return false;

                        // Check for valid TDS readings (>10 ppm is a safe floor to filter out dummy 0 data)
                        const hasValidTds = (g.ro_reject?.tds > 10) ||
                            (g.rainwater?.tds > 10) ||
                            (g.blended?.tds > 10);
                        return hasValidTds;
                    });
